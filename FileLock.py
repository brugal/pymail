...
def fcntl_lock_file_up_to_present_bytes (fileObject):
	...
	print "fcntl_lock_file_up_to_present_bytes ", fileObject
	size = os.stat(fileObject.name)[stat.ST_SIZE]
	_fcntl_lock (fileObject, size)

def fcntl_lock_entire_file (fileObject):
	"""fcntl() locks the entire file so that other programs can't event 
	append to the file."""
	print "fcntl_lock_entire_file ", fileObject
	_fcntl_lock (fileObject, 0)

def fcntl_remove_lock (fileObject):
	print "fcntl_remove_lock ", fileObject
	lockdata = struct.pack ('hhllhh', FCNTL.F_WRLCK, 0, FCNTL.SEEK_SET,
				0, 0, 0)
